...
class FileLogger:
    ...
    def log_request(self, request_data: Dict) -> bool:
        """
        Log API request data to CSV file.
        ...
        try:
            timestamp = datetime.now().isoformat()
            sender_name = request_data.get('name', 'Unknown')
            email_content = request_data.get('content', '')
            reply = request_data.get('reply', '')
            status = request_data.get('status', 'processed')
            ...
            print(f"Request logged to CSV: {self.senders_file}")
            return True
            
        except Exception as e:
            print(f"Error logging request to CSV file: {e}")
            return False
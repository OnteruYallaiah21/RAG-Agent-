...
Description:
------------
Configuration management for Thryvix Email Agent with modern architecture.
...
4. Manages web server and email service settings
5. Handles caching and notification configurations
"""
...
class Settings(BaseSettings):
    ...
    from_email: str = Field("noreply@thryvix.ai", env="FROM_EMAIL")
    from_name: str = Field("Thryvix AI", env="FROM_NAME")
    
    # Google Services
    google_sheets_credentials: Optional[str] = Field(None, env="GOOGLE_SHEETS_CREDENTIALS")
    google_spreadsheet_id: Optional[str] = Field(None, env="GOOGLE_SPREADSHEET_ID")
    ...
    notification_retention_days: int = Field(30, env="NOTIFICATION_RETENTION_DAYS")
